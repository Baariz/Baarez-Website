...
const PartnerOnboard = () => {
  ...
  const handleSubmit = (e) => {
    ...
    .then(() => {
      setStatus({ type: 'success', message: 'Application submitted successfully. Our channel team will review and contact you.' });
      setIsSubmitting(false);
      setStep(4); // Final Success Step
    }, (error) => {
      setStatus({ type: 'error', message: 'Submission failed. Please try again or contact info@baarez.com.' });
      setIsSubmitting(false);
    });
  };
      ...
      <section className="bg-white py-16 border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-6">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center">
            <span className="text-[#ef7f25] text-[10px] font-bold uppercase tracking-[0.4em] mb-4 block">Channel Program</span>
            <h1 className="text-3xl md:text-4xl font-bold text-[#760015] mb-6">Partner Onboarding Application</h1>
            <p className="text-gray-500 text-sm max-w-2xl mx-auto leading-relaxed">
              Join the Baarez ecosystem. Please provide accurate organizational details to begin 
              the validation and onboarding process.
            </p>
          </motion.div>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="bg-white border border-gray-100 shadow-xl p-8 md:p-12">
              ...
              <AnimatePresence mode="wait">
                ...
                {step === 1 && (
                  <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                    <h3 className="text-[#760015] font-bold text-lg border-b border-gray-50 pb-4">Primary Classification</h3>
                    
                    <div className="grid md:grid-cols-2 gap-8">
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Partner Type *</label>
                    ...
                    <div className="space-y-2">
                      ...
                      <input required type="text" name="country" placeholder="e.g. Qatar, UAE" className="w-full px-4 py-3 bg-[#fcfcfc] border border-gray-100 focus:border-[#ef7f25] outline-none text-sm" />
                    </div>

                    <div className="pt-6 flex justify-end">
                      <button type="button" onClick={nextStep} className="bg-[#760015] text-white px-10 py-3 text-[10px] font-bold uppercase tracking-widest hover:bg-[#ef7f25] transition-all">Next</button>
                    </div>
                  </motion.div>
                )}
                ...
                {step === 2 && (
                  <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                    <h3 className="text-[#760015] font-bold text-lg border-b border-gray-50 pb-4">Legal & Business Capacity</h3>
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Doing Business As (DBA) *</label>
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Years in Business *</label>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Total Workforce Count *</label>
                    ...
                    <div className="pt-6 flex justify-between">
                      <button type="button" onClick={prevStep} className="text-gray-400 text-[10px] font-bold uppercase tracking-widest hover:text-[#760015]">Back</button>
                      <button type="button" onClick={nextStep} className="bg-[#760015] text-white px-10 py-3 text-[10px] font-bold uppercase tracking-widest hover:bg-[#ef7f25] transition-all">Next</button>
                    </div>
                  </motion.div>
                )}
                ...
                {step === 3 && (
                  <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                    <h3 className="text-[#760015] font-bold text-lg border-b border-gray-50 pb-4">Professional Resources</h3>
                    
                    <div className="grid md:grid-cols-2 gap-8">
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Technical Employees *</label>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Sales Employees *</label>
                    ...
                    <div className="pt-6 flex justify-between items-center">
                      <button type="button" onClick={prevStep} className="text-gray-400 text-[10px] font-bold uppercase tracking-widest hover:text-[#760015]">Back</button>
                      <button type="submit" disabled={isSubmitting} className="bg-[#760015] text-white px-12 py-4 text-[10px] font-bold uppercase tracking-widest hover:bg-[#ef7f25] transition-all disabled:bg-gray-400">
                ...
                {step === 4 && (
                  <motion.div key="success" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-10 space-y-6">
                    ...
                    <h3 className="text-2xl font-bold text-[#760015]">Application Received</h3>
                    ...
                    <button type="button" onClick={() => window.location.href = '/'} className="text-[#ef7f25] font-bold text-[10px] uppercase tracking-widest border-b border-[#ef7f25]">Return to Home</button>
                  </motion.div>
                )}
            ...
            {status.message && step !== 4 && (
              <p className={`mt-6 text-center text-[10px] font-bold uppercase ${status.type === 'success' ? 'text-green-600' : 'text-red-600'}`}>
                {status.message}
              </p>
            )}
